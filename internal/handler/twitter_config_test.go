...
func TestLoadTwitterConfig_WithEnv(t *testing.T) {
	...
	t.Setenv("TWITTER_ALLOWED_HOSTS", "misskey.io, mi.example.com")
	...
	assert.Contains(t, config.AllowedHosts, "misskey.io")
...
func TestTwitterConfig_CheckEligibility_Disabled(t *testing.T) {
	...
	result := config.CheckEligibility(true, "misskey.io")
...
func TestTwitterConfig_CheckEligibility_AllowedHosts(t *testing.T) {
	config := TwitterConfig{
		...
		ClientSecret:   "secret",
		RequireMisskey: true,
		AllowedHosts:   []string{"misskey.io", "mi.soli0222.com"},
	}
	...
	tests := []struct {
		...
		{"exact match", "misskey.io", true},
		{"exact match 2", "mi.soli0222.com", true},
		{"with https", "https://misskey.io", true},
		{"with http", "http://misskey.io", true},
		{"with trailing slash", "misskey.io/", true},
